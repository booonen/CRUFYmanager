...
## 9. Polish round (2026-06-10): style modifiers

The original brief's style mods (missed when plan 2 absorbed it; user caught it) are in. `Entry.styleMod` (−5…+5, decimals), combined per fixture and applied as a **mutual goal shift**: positive combined style adds Poisson-sampled goals to *both* sides, negative removes them capped at the loser's count — so a +5 pair turns 2–1 into ~5–4, a −5 pair into 1–0, and the winner/GD are preserved *by construction* (test-locked across seeds; zero combined style is bit-identical to no style). `SimParams.styleImpact` (default 0.35 mutual goals per combined point) tunes the strength. Style is editable in the Entries tab, parsed as the second number in wizard paste lines (`Name, CODE, rating, style`), visible in the Sim Lab (Style A/B inputs), and part of the inputs digest (drift detection). Backfilled to 0 on old saves.
