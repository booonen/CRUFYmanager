...
## 11. Polish round (2026-06-10, after the user's first hands-on)

User feedback → same-day changes:

12. **Decimal seeding.** `Entry.seeding` is a plain number: higher = better, decimals allowed, scale-free (KPB-style ~0–30 football ranks, 0–100 Olympic ratings). The `rank | rating` mode split from §2 was a misreading of NS conventions and is removed; a load-time baseline normalizer converts old saves (rank values inverted within their event). Wizard paste format is now `Name, CODE, rating`; unrated lines get 0 and keep paste order among themselves (stable sort). Phase 3 must treat rating gaps scale-awarely (per-save spread parameter) — recorded in the plan.
13. **Matchday-focused cockpit.** A round rail (status glyphs ○ ◐ ● ✓, compact `MD{n}` labels) focuses exactly one round; group stages section the matchday's fixtures per group; the standings panel shows the **cumulative table as of the focused round** (`computeGroupTablesThrough` / `computeOverallTableThrough`). Default focus = first unfinished round.
14. **Results-post BBCode.** Rewritten to the community shape (modeled on a real NSWC post the user supplied): per-group `[box]` → `[size=125][b]Group N[/b][/size]` → `[hr][/hr][b]MD{n}[/b]` → plain result lines (`(aet)` / `(x–y pens)` suffixes) → `[pre]` monospace cumulative table with Pld/W/D/L/GF/GA/GD/Pts, `=` position markers for rows tied on pts+GD, U+2212 for negatives. `matchdayPostBBCode` is the flagship copy ("Copy matchday post" on every round card); `[table]` markup is gone. This sample is the Phase 4 pipeline's baseline artifact.
15. **Two-lens calendar.** User clarification: the cross-competition grid is core ("It's not a calendar per competition"). `Round.calendarMatchday: number | null` (auto-assigned consecutively from the current matchday in `addCompetition`, reassignable via an MD field on every round card, 0 = unscheduled, overflow past season end → unscheduled list). New `engine/calendar.ts` (`matchdayIndex`, `unscheduledRounds`); Calendar route = matchday grid (chips per competition, current highlighted, double-click sets current, Advance button) + selected-day panel rendering each competition's round as a live RoundCard. Plan-1 `ScheduleSlot`/`ScheduledFixture`/`Calendar.schedule` deleted; Dashboard fixture count derives from scheduled rounds. Old saves get matchdays backfilled 1..n per competition.
16. **Calendar v2 (second polish pass).** Calendar is **overview & planning only — never score entry** (user direction). The grid fills the page: per-day cells list round chips (sport icon · competition code · round label · status glyph ○ ◐ ● ✓), current day badged. Clicking a day opens `/calendar/:md`, a **read-only programme**: one block per competition-round (status + result count, fixture list with scores/dashes, aet/pens suffixes), and clicking a block deep-links to `/competitions/:id?stage=…&round=…` — the cockpit honors those params to select the stage tab and focus the matchday. **Planning is drag & drop**: chips drag between days; an Unscheduled tray accepts drops (unschedule) and drags back out. Round-status helpers (`roundStatus`, `STATUS_GLYPH`, `compactRoundLabel`) moved to `engine/calendar.ts`; smoke tests assert the grid renders draggable chips and the overview contains zero `<input>`s.
